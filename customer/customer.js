...
async function populateDeviceTable() {
  ...
  filteredDevices.forEach(device => {
    ...
    const connectionStatus = `<span class="connection-status connection-${device.is_connected ? 'online' : 'offline'}"><span class="connection-dot"></span>${device.is_connected ? 'online' : 'offline'}</span>`;
...
async function applyFilters() {
  ...
  filteredDevices.forEach(device => {
    ...
    if (needsUpdate) {
      ...
      statusBadge = '<span class="status-badge status-up-to-date">Up to Date</span>';
    }
    row.innerHTML = `
      <td><input type="checkbox" class="device-checkbox" data-device-id="${device.device_id}"></td>
      <td data-label="ID">${device.device_id}</td>
